            ...
            <div class="container">
                <div class="row">
                    <div class="col-lg-8 col-xl-6 text-center mx-auto">
                        ...
                        <h1 class="mb-4">Завершение курса</h1>
                    </div>
                </div>

                <div class="container my-3">
                    @if($res == -1)
                        <div class="alert alert-danger">К сожалению, вы набрали меньше 50%!</div>

                        <a href="{{route('passExam', $id)}}" class="btn btn-outline-primary">Пройти снова</a>
                    @elseif($res == 0)
                        <form action="{{route('checkExam')}}" method="post">
                        @csrf
                            ...
                            @foreach($quizzes as $quiz)
                                <div class="container">
                                    ...
                                    <div class="form-check">
                                        <input value="{{$quiz->a}}" class="form-check-input" type="radio" name="question-{{$quiz->id}}" id="flexRadioDefault1{{$quiz->id}}" checked>
                                        <label class="form-check-label" for="flexRadioDefault1{{$quiz->id}}">
                                            {{$quiz->a}}
                                        </label>
                                    </div>
                                    <div class="form-check">
                                        <input value="{{$quiz->b}}" class="form-check-input" type="radio" name="question-{{$quiz->id}}" id="flexRadioDefault2{{$quiz->id}}">
                                        <label class="form-check-label" for="flexRadioDefault2{{$quiz->id}}">
                                            {{$quiz->b}}
                                        </label>
                                    </div>
                                    <div class="form-check">
                                        <input value="{{$quiz->c}}" class="form-check-input" type="radio" name="question-{{$quiz->id}}" id="flexRadioDefault3{{$quiz->id}}">
                                        <label class="form-check-label" for="flexRadioDefault3{{$quiz->id}}">
                                            {{$quiz->c}}
                                        </label>
                                    </div>
                                    <div class="form-check">
                                        <input value="{{$quiz->d}}" class="form-check-input" type="radio" name="question-{{$quiz->id}}" id="flexRadioDefault4{{$quiz->id}}">
